...
float map_blendfactor_ground_var	= 0.5f;		// detailmap blending strength over base colormap
...
texture entSkin2;		// detail colormap
//texture entSkin3;
//texture entSkin4;
...
//	float4 Emissive = vecEmissive;	
   ...
   float4 Color = 1;
//   Color.rgb = lerp(color.rgb, color1.rgb, map_blendfactor_ground_var);										
//   //lerp(x,y,s) = x*(1-s) + y*s;	 0.67 means stronger tile blending
	Color.rgb = (color.rgb + color1.rgb) - 0.5;						// non-shader compatible solution
...
#ifdef PER_PIXEL_LIGHTS	
	Color.rgb *= (Diffuse + Specular + Lights + InColor);			// with per pixel dynamic lights, InColor contains only entity ambient fAmbient
#else
