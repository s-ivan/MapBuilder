...
//	color7.a 	  = tex2D(tileSampler7, Tex).a;
	...
	float summedweight = color2.a + color3.a;
...
//	float4 Specular = pow(saturate(dot(R, InViewDir)), fPower) * vecSpecular * (vecLight + fAlbedo);									
	...
	// texture
   float4 Color = 1;
   Color.rgb = lerp(color.rgb, clamp((color1.rgb*color1.a + color2.rgb*color2.a + color3.rgb*color3.a)/summedweight,0,1), map_blendfactor_ground_var);										
