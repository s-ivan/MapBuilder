...
//							reset( ent , OVERLAY );
					...
					ent.material 	= mtl_mapent;
					int i;
					for(i=0;i<ent_status(ent,8);i++)
						{
							ent_mtlset(ent, mtl_mapent, i+1);		// needed for all skins only in case of map entities!
						}
...
//							reset( ent , UNLIT );
					...
					ent.ENT_CATEGORY = (var)UIMODE_MAPENT;
...
//					ptr_remove(temp5_str);	
					...
					else if (((shadow_stencil>(var)-1) && (shadow_stencil<(var)8)) || (my_flag5>0))	//(obs_collision==(var)2)		// flag5 means mapent_decal
						{
							// to set bounding box to extends because of min_z and max_z 
							// or before bounding box vec_scale is used	
							set(ent, PASSABLE);							
							
							MapEnt_CreateCollider(ent);
						}									
					...
					MapEnt_SetShadow(ent, (1-my_flag6), my_flag5, obs_shadow);
					wait_for(MapEnt_SetShadow);													// needed because of decal and ent_decal shadows, to wait for completion before setting entity to static
...
//					MapEnt_Created_Loop(ent);															// it can result in too many running functions !!! and entity cannot be static
...
//					Terrain_SetBuildingTilesClusters(ent, mapent_foldername);					// done for all buildings on game init
//	#endif
//#endif
					//---------------------------------------------------------------------------------------------------
				}
...
void		MapEnt_CreateCollider(ENTITY* ent)
{	
...
//	obs_collider.pan = ent.pan;																					// ONLY IN FREE EDITION - IT SHOULD BE 0,90,180 OR 270 DEGREE ONLY !!!!!!!!!!!!!!!
	...
	obs_collider.ENT_CATEGORY = (var)UIMODE_MAPENT + (var)UIMODE_SHADOWSHIFT;
...
//	obs_collider.min_x = -GetMapTileSize()/2 * mapent_tilesize_x * mapent_scale;					// required ?
...
//void		MapEnt_Created_Loop(ENTITY* ent)
//{
											...
											)
{
	...
	if (file_handle)
	...
	else
		{
			...
			*obs_collision			= 1;								// 0 passable, 1 polygonal (for real buildings), 2 tile area (cheaper, for blocking objects)
...
void	MapEnt_SetSprite(ENTITY* ent, var mpe_decal)						// called only in case of shadow_stencil 0 and 1
{	
	if ((!main_blur) && (mpe_decal<1))												// no stencil blur applied, no entdecal enforced
		{
			MapEnt_SetDecal(ent);
			wait_for(MapEnt_SetDecal);
		}
	else
		{
			if (mpe_decal)
			...
			MapEnt_SetEntDecal(ent);
			wait_for(MapEnt_SetEntDecal);
		}	
}
...
{	
	if (mpe_shadow == (var)0)														// no shadow
		{
			MapEnt_SetNoshadow(ent);
			wait_for(MapEnt_SetNoshadow);
		}
	else if (mpe_lightmap == (var)1)												// static lightmap shadow
		{
			MapEnt_SetLightmap(ent);
			wait_for(MapEnt_SetLightmap);
		} 
	else																					// dynamic shadow
		{						
			if (shadow_stencil == (var)-1)										// sc esm shadowmapping
				...
				{					
//					if (mpe_shadow==(var)1)											// stencil shadow	- not applicable for map entities !!!
//						{
//							if (mpe_decal<1)
//								{
//									MapEnt_SetStencil(ent);
//									wait_for(MapEnt_SetStencil);
//								}
//							else															// enforce sprite shadow (decal or entdecal)
//								{
//									MapEnt_SetSprite(ent, mpe_decal);
//									wait_for(MapEnt_SetSprite);
//								}
//								
//						}
//					else 
					if (mpe_shadow==(var)2)											// sprite shadow (decal or entdecal)
						{
							MapEnt_SetSprite(ent, mpe_decal);
							wait_for(MapEnt_SetSprite);
						}
					else if (mpe_shadow==(var)3)									// model shadow
						{
							if (mpe_decal<1)
								{
									MapEnt_SetModel(ent);
									wait_for(MapEnt_SetModel);
								}
							else															// enforce sprite shadow (decal or entdecal)
								{									
									MapEnt_SetSprite(ent, mpe_decal);
...
{}
	...
	//---------------------------------------------
	// entity count
	str_for_num( 			temp_str 	, mapent_counter );
	...
	you = ent_next(NULL); 									// get first entity
 	while (you) 												// repeat until there are no more entities
		{
			if ((you.ENT_CATEGORY == (var)UIMODE_MAPENT) && (!is(you,FLAG8)))				// if map entity, not part of wmb
				{
					// name
					...
					file_str_write( 		file_handle , temp_str );
						file_asc_write(	file_handle , 13); 
						file_asc_write(	file_handle , 10);	
					// material: shaded or category specific - okay
					if (you.material==mat_shaded)	
						{
							str_for_num( 	temp_str 	, 1 );
						}	
//					else	if (you.material==mtl_mapent_transp)	
//						{
//							str_for_num( 	temp_str 	, 2 );
//						}	
					else	// if (you.material==mtl_mapent)	
						{
							str_for_num( 	temp_str 	, 0 );
						}
					file_str_write( 		file_handle , temp_str );
...
//	printf( "MapEnt Map Saved: %s" , _chr(file_name) );
...
{}
	...
	//mapent_counter += str_to_int( temp_str );		// set during creation - any object creation could fail
	...
	while(1)		// or for loop based on count value read in
		{
			...
			while (proc_status (MapEnt_CreateItem_FromFile) > 2000)
				{
					wait(10);
				}
			// create entity
			MapEnt_CreateItem_FromFile( import, my_filename, 
...
{}
	...
	file_close(file_handle);
	//----------------------------------------------
	//printf( "MapEnt Map Loaded: %s" , _chr(file_name) );
...
void MapEnt_CreateItem_FromFile(var imported, STRING* my_filename, 
										var my_scale_x, my_scale_y, my_scale_z,
										my_x, my_y, my_z, 
										my_pan, my_tilt, my_roll,
										my_alpha, my_ambient, 
...
//	my_filename = str_lwr( my_filename ); 		
	...
	if ( file_exists(temp_str) )		// slow double check - not essential
		{			
			//-----------------------------------------------------------------------
			// set mapent properties from .obs file 
			...
			MapEnt_ReadObsFileData(my_filename, &obs_defaultsize, &obs_tilesize_x, &obs_tilesize_y, &obs_type, &obs_collision, &obs_shadow);
...
function MapEnt_RemoveObject(ENTITY* ent)
{
	if (ent == NULL)
		{
			return 0;
	...
	if (ent.ENT_CATEGORY != (var)UIMODE_MAPENT)
		{
			return 0;
		}
...
void		MapEnt_RemoveChild(ENTITY* owner)
{
	you = ent_next(NULL); 
 	while (you) 	
		{
			if ( (you.XTILE_POS == owner.XTILE_POS) && (you.YTILE_POS == owner.YTILE_POS) )
				{
					if (you.ENT_CATEGORY == ((var)UIMODE_MAPENT + (var)UIMODE_SHADOWSHIFT) )
						{
							if (you.parent == owner)
								{
									ent_remove(you);
...
int		MapEnt_RemoveArea(int midtilex, int midtiley, int areasizex, int areasizey)
{
	int i,j;
	var k = 1;
 	...
 	while (you) 	
		{
			...
			if ( (you.XTILE_POS >= tempx) && (you.XTILE_POS < tempx + areasizex) && (you.YTILE_POS >= tempy) && (you.YTILE_POS < tempy + areasizey) )		// here could be an area check -> would be much faster
				{
					if (you.ENT_CATEGORY == (var)UIMODE_MAPENT)
						{
							// store next entity
					...
					else if (you.ENT_CATEGORY == ((var)UIMODE_MAPENT + (var)UIMODE_SHADOWSHIFT) )
						{
							// store next entity
							temp_ent = ent_next(you); 
...
void		MapEnt_RemoveAll()
{
	Remove_EntCats( UIMODE_MAPENT );
