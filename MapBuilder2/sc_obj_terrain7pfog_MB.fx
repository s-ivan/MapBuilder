...
#ifndef HEIGHT_FOG   
	float Fog = vecFogColor.w																									// fog is applied
				 * saturate( (distance(In.WorldPos, vecViewPos) - vecFog.x) * (vecFog.z) );					// saturate needed when fadeout distance is closer than clipping distance
#else
	float Fog = VS_heightfog(In.WorldPos.xyz);
#endif
