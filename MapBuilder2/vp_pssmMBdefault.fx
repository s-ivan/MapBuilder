...
{}  
	...
	if(fDistance < pssm_splitdist_var[1] || pssm_limit_var < 2)
	...
	#ifdef OWM_BIAS
		if(fDistance < pssm_splitdist_var[1] || pssm_limit_var < 2)							// use entity specific pcf and bias values
		...
		if(fDistance < pssm_splitdist_var[1] || pssm_limit_var < 2)
			{
				fShadow = DoPCF(sDepth1, TexCoord[0], PCFSAMPLES_NEAR, pssm_fbias_flt);				// PCFSAMPLES_NEAR 1*
				{}
				#ifdef PSSM_DEBUG					
...
//	float alpha = tex2D(sBaseTex,inTex.xy).a * pssm_transparency_var;
	...
	// min max fade out 
	alpha *= ( 1 - saturate( (fDistance-pssm_fadeoutstart_var) / (pssm_fadeoutend_var-pssm_fadeoutstart_var) ) );		// pssm_fadeoutstart_var pssm_fadeoutend_var fDistance
